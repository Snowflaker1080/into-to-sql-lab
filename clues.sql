-- Terminal command psql -d world -f clues.sql
...
HAVING COUNT(*) = 1; 
-- The HAVING COUNT(*) = 1 ensures the country has only one language entry (i.e., speaks only Italian).
-- Italy & San Marino
...
-- Query finds the city in San Marino that is not named "San Marino".
...
SELECT capital_city.name AS capital_city
FROM countries
JOIN cities ON cities.name = 'Serra' AND cities.countrycode = countries.code
JOIN cities AS capital_city ON countries.capital = capital_city.id;
-- Brasília, Brazil
